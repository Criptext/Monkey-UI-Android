...
public class CircularAudioView extends View {
	...
	private static final int DEFAULT_POINTER_COLOR = Color.argb(255, 255, 255, 255);
    ...
    private static final float DEFAULT_INNER_TOLERANCE = 0f;
	...
	private static final int DEFAULT_CLICK_THRESHOLD = 400;
	...
	/**
	 * Start angle of the CircularAudioView.
	...
	/**
	 * End angle of the CircularAudioView.
	 * Note: If mStartAngle and mEndAngle are set to the same angle, 0.1 is subtracted
	 * from the mEndAngle to make the circle function properly.
	 */
	...
	/**
	 * Max value that this CircularAudioView is representing.
	 */
	private int mMax;

	/**
	 * Progress value that this CircularAudioView is representing.
	 */
	private int mProgress;

	/**
	 * If true, then the user can specify the X and Y radii.
	 * If false, then the View itself determines the size of the CircularAudioView.
	 */
	 ...
	 * Used when touching the CircularAudioView.
	 */
	...
	private float mInnerTolerance;

	private AudioActions mAudioActions;
	/**
	 * Listener.
	 */
	private OnCircularAudioViewChangeListener mOnCircularAudioViewChangeListener;

	/**
	 * Initialize the CircularAudioView with the attributes from the XML style.
	...
	private void initAttributes(TypedArray attrArray) {
		mCircleXRadius = (float) (attrArray.getFloat(R.styleable.CircularAudioView_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS) * DPTOPX_SCALE);
		mCircleYRadius = (float) (attrArray.getFloat(R.styleable.CircularAudioView_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS) * DPTOPX_SCALE);
		mPointerRadius = (float) (attrArray.getFloat(R.styleable.CircularAudioView_pointer_radius, DEFAULT_POINTER_RADIUS) * DPTOPX_SCALE);
		mPointerHaloWidth = (float) (attrArray.getFloat(R.styleable.CircularAudioView_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH) * DPTOPX_SCALE);
		mPointerHaloBorderWidth = (float) (attrArray.getFloat(R.styleable.CircularAudioView_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH) * DPTOPX_SCALE);
		mCircleStrokeWidth = (float) (attrArray.getFloat(R.styleable.CircularAudioView_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH) * DPTOPX_SCALE);
		mInnerTolerance = Math.min((float) (attrArray.getFloat(R.styleable.CircularAudioView_inner_tolerance, DEFAULT_INNER_TOLERANCE)),1)*mCircleStrokeWidth;

		String tempColor = attrArray.getString(R.styleable.CircularAudioView_pointer_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.CircularAudioView_pointer_halo_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.CircularAudioView_pointer_halo_color_ontouch);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.CircularAudioView_circle_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.CircularAudioView_circle_progress_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.CircularAudioView_circle_fill);
		if (tempColor != null) {
			try {
		...
		mPointerAlphaOnTouch = attrArray.getInt(R.styleable.CircularAudioView_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH);
		if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
			mPointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ONTOUCH;
		}

		mMax = attrArray.getInt(R.styleable.CircularAudioView_max, DEFAULT_MAX);
		mProgress = attrArray.getInt(R.styleable.CircularAudioView_progress, DEFAULT_PROGRESS);
		mCustomRadii = attrArray.getBoolean(R.styleable.CircularAudioView_use_custom_radii, DEFAULT_USE_CUSTOM_RADII);
		mMaintainEqualCircle = attrArray.getBoolean(R.styleable.CircularAudioView_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE);
		mMoveOutsideCircle = attrArray.getBoolean(R.styleable.CircularAudioView_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE);
        lockEnabled = attrArray.getBoolean(R.styleable.CircularAudioView_lock_enabled, DEFAULT_LOCK_ENABLED);

		// Modulo 360 right now to avoid constant conversion
		mStartAngle = ((360f + (attrArray.getFloat((R.styleable.CircularAudioView_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
		mEndAngle = ((360f + (attrArray.getFloat((R.styleable.CircularAudioView_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
		//System.out.println("BARRIENDO : " +  mStartAngle + "-" + mEndAngle);
...
*/
	...
	protected void onDraw(Canvas canvas) {
		...
		//canvas.drawBitmap( playBmp, new Rect(0,0,123,122), new RectF(-61f*DPTOPX_SCALE,-61f*DPTOPX_SCALE,61*DPTOPX_SCALE,61f*DPTOPX_SCALE), null);
	...
	/**
	 * Get the progress of the CircularAudioView.
	 * @return The progress of the CircularAudioView.
	 */
	...
	/**
	 * Set the progress of the CircularAudioView.
	 * If the progress is the same, then any listener will not receive a onProgressChanged event.
	 * @param progress The progress to set the CircularAudioView to.
	 */
	public void setProgress(int progress) {
		if (mProgress != progress) {
			mProgress = progress;
			if (mOnCircularAudioViewChangeListener != null) {
				mOnCircularAudioViewChangeListener.onProgressChanged(this, progress, false);
			}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		//int height = MeasureSpec.makeMeasureSpec((int)(mCircleStrokeWidth*mViewSize), MeasureSpec.EXACTLY);
		//int width = MeasureSpec.makeMeasureSpec((int)(mCircleStrokeWidth*mViewSize), MeasureSpec.EXACTLY);
		...
		mCircleHeight = 40*this.DPTOPX_SCALE;// (float)height / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 1.5f);
		mCircleWidth = 40*this.DPTOPX_SCALE;//(float)width / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 1.5f);
		
		//Downscale to get size I want
		//mCircleHeight /= ARBITRARY_DOWNSCALE;
		//mCircleWidth /= ARBITRARY_DOWNSCALE;
		...
		//System.out.println("SUGGESTED: " + getSuggestedMinimumHeight() + " ACTUAL: " + height + "mCircleHeight: " + mCircleHeight );
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		float innerRadius = Math.min(mCircleHeight, mCircleWidth) - additionalRadius + mInnerTolerance; // Min inner radius of the circle, including the minimumTouchTarget or wheel width
		//System.out.println("InnerTolerance: " + mInnerTolerance);
		...
		case MotionEvent.ACTION_DOWN:
			...
			if(touchEventRadius < innerRadius){
				...
				this.postDelayed(new Runnable() {
					...
					public void run() {
						if(lastPress != -1)
							mAudioActions.onActionLongClicked();
					}
				}, DEFAULT_CLICK_THRESHOLD);
			} else if (((touchEventRadius >= innerRadius) && (touchEventRadius <= outerRadius)) && ( (cwDistanceFromPointer <= pointerRadiusDegrees) || (ccwDistanceFromPointer <= pointerRadiusDegrees)) ) {
				...
				invalidate();
				if (mOnCircularAudioViewChangeListener != null) {
					mOnCircularAudioViewChangeListener.onStartTrackingTouch(this);
				}
				...
				if (mOnCircularAudioViewChangeListener != null) {
					mOnCircularAudioViewChangeListener.onStartTrackingTouch(this);
					mOnCircularAudioViewChangeListener.onProgressChanged(this, mProgress, true);
				}
				mUserIsMovingPointer = true;
				lockAtEnd = false;
				lockAtStart = false;
		...
		case MotionEvent.ACTION_UP:
			//mPointerHaloPaint.setAlpha(mPointerAlpha);
			//mPointerHaloPaint.setColor(mPointerHaloColor);
			if(lastPress !=-1 && System.currentTimeMillis() - lastPress < DEFAULT_CLICK_THRESHOLD){
				//DO PLAY/PAUSE ACTION
				mAudioActions.onActionClicked();
				lastPress = -1;
			} else	if (mUserIsMovingPointer) {
				mUserIsMovingPointer = false;
				invalidate();
	...
	private void init(AttributeSet attrs, int defStyle) {
		final TypedArray attrArray = getContext().obtainStyledAttributes(attrs, R.styleable.CircularAudioView, defStyle, 0);
	...
	public CircularAudioView(Context context) {
		super(context);
		init(null, 0);
	}

	public CircularAudioView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public CircularAudioView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setOnSeekBarChangeListener(OnCircularAudioViewChangeListener l) {
		mOnCircularAudioViewChangeListener = l;
	}

	/**
	* Listener for the CircularAudioView. Implements the same methods as the normal OnSeekBarChangeListener.
	*/
	public interface OnCircularAudioViewChangeListener {

		public abstract void onProgressChanged(CircularAudioView CircularAudioView, int progress, boolean fromUser);

		public abstract void onStopTrackingTouch(CircularAudioView seekBar);

		public abstract void onStartTrackingTouch(CircularAudioView seekBar);
	}
	...
	public void setAudioActions(AudioActions aa){
		mAudioActions = aa;
	}
	/**
	...
	/**
	 * Set the max of the CircularAudioView.
	 ...
	 * @param max The new max for the CircularAudioView.
	 */
	public void setMax(int max) {
		if (!(max <= 0)) { // Check to make sure it's greater than zero
			if (max <= mProgress) {
				...
				if (mOnCircularAudioViewChangeListener != null) {
					mOnCircularAudioViewChangeListener.onProgressChanged(this, mProgress, false);
				}
			}
			mMax = max;
	...
	/**
	 * Get the current max of the CircularAudioView.
